...
© 2024 Document Sharing. All rights reserved.
        ...
        public async Task SendPasswordResetEmailAsync(string toEmail, string resetLink)
        {
            try
            {
                ...
                var to = new EmailAddress(toEmail);
                var subject = "Đặt lại mật khẩu - Document Sharing";
...
<body>
   ...
   '>
    Đặt lại mật khẩu
            ...
            <p>Nếu bạn không thể nhấp vào nút, hãy sao chép và dán link sau:</p>
            <p style='word-break: break-all; color: #667eea; font-size: 14px;'>{resetLink}</p>

            <div class='info-box'>
                <p><strong>⚠️ Lưu ý:</strong> Link đặt lại mật khẩu sẽ hết hạn sau <strong>1 giờ</strong>.  
                Nếu bạn không yêu cầu chức năng này, vui lòng bỏ qua email.</p>
            </div>
        </div>
...
";
                ...
                // Plain text version
                var plainTextContent = $@"
Đặt lại mật khẩu - Document Sharing
...
© 2024 Document Sharing. All rights reserved.
                ...
                {
                    var responseBody = await response.Body.ReadAsStringAsync();
                    _logger.LogError($"Failed to send password reset email. Status: {response.StatusCode}, Body: {responseBody}");
                    throw new Exception($"Failed to send email: {response.StatusCode}");
                }
            }
            catch (Exception ex)
